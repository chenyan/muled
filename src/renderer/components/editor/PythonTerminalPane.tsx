...
interface PythonTerminalPaneProps {
  sessionId: string;
...
function schedulePtyResize(
  ...
  void window.muled.python.pty.resize({ sessionId, cols, rows });
}
...
export default function PythonTerminalPane({
  sessionId,
  resizeFrozen = false,
  onExit,
  ...
  useEffect(() => {
    const unsubData = window.muled.python.pty.onData(
      ({ sessionId: id, data }) => {
        if (id !== sessionIdRef.current) return;
        writeFromPty(data);
      },
    );
    const unsubExit = window.muled.python.pty.onExit(
  ...
  const handleData = useCallback((data: string) => {
    void window.muled.python.pty.write({
